...
2. 下载 `vohive-plus-desktop_1.0.2_windows_x64.zip`。
...
- `vohive-plus-desktop_1.0.2_windows_x64.zip`：Windows x64 桌面便携包，内含桌面壳、Linux amd64 后端运行时、默认配置和 WSL USB 准备脚本。
- `vohive-plus-firmware_1.0.2_linux_amd64`：Linux amd64 后端运行时，适用于 WSL2 或 Linux x86_64 主机。
...
GOWORK=off CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
  ...
  -ldflags "-s -w -X 'github.com/swordstudiox/vohive-plus/internal/global.Version=1.0.2'" \
...
```bash
git tag -a v1.0.2 -m "发布 v1.0.2"
git push origin main
git push origin v1.0.2
```

GitHub Actions 会构建后端多架构运行时、Windows 桌面便携包，并读取 `.github/release-notes/v1.0.2.md` 发布到 [vohive-plus/releases](https://github.com/swordstudiox/vohive-plus/releases)。
